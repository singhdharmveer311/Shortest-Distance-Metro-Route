...
void graph::addEdge(string src , string dst , int distance){
    neighbor tmp_node;
    //from source  to destination path:
...
void graph::dijkstra(string src , string dst){
    if(src == dst){
        ...
        for(auto it = map.begin() ; it != map.end() ; it++){
            distances[it->first].first = 2143658710654;//intialize all the distances with "infinity"
            distances[it->first].second = "N/A";//and still you didn't reach any of them
        }
